...
int BPlusTree::bPlusInsert(int relId, char attrName[ATTR_SIZE], Attribute attrVal, RecId recId) {
    ...
    /* blocknum=rootBlock of B+ Tree (from attrCatEntry) */
    int blockNum = cacentry.rootBlock;
...
int BPlusTree::findLeafToInsert(int rootBlock, Attribute attrVal, int attrType) {
    ...
    while (StaticBuffer::getStaticBlockType(blockNum) != IND_LEAF) {  
        ...
        for(i=0;i<inthead.numEntries;i++){
            ...
            int cmpval=compareAttrs(attrVal,entry.attrVal,attrType);
            if(cmpval<=0){
                break;
            }
        }
   ...
   for(int i=0;i<blockHeader.numEntries;i++){
    ...
    if(compareAttrs(entry.attrVal,indexEntry.attrVal,cacentry.attrType)<=0){
        ...
        for(i++;i<=blockHeader.numEntries;i++){
            leaf.getEntry(&indices[i],i-1);
        }
        break;
    }
   }
   if(!flag) indices[blockHeader.numEntries]=indexEntry;
